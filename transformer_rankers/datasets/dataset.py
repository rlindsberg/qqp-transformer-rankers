...
def collate_T2T_batch(batch: List) -> Dict[str, torch.Tensor]:
    ...
    decoder_attention_mask = torch.stack(
        [torch.tensor(example['target_attention_mask'], dtype=torch.long) for example in batch])
    ...
    def _cache_instances_pickle(self):
        ...
        if os.path.exists(path):
            ...
            for r in self.data.itertuples(index=False):
                ...
                labels += ([not_relevant_label] * (
                    self.negative_sampler.num_candidates_samples))  # each query has N negative samples.
    ...
    def _cache_instances_memmap(self):
        ...
        if not os.path.exists(path_input_ids):
            ...
            for k in ['input_ids', 'token_type_ids', 'attention_mask']:
                ...
                self.mem_maps[k] = np.memmap(path_input, dtype='int', mode='w+',
                                             shape=(labels.shape[0], self.max_seq_len))
    ...
    def _cache_instances_memmap(self):
        ...
        if not os.path.exists(path_input_ids):
            ...
            for k in ['input_ids', 'token_type_ids', 'attention_mask']:
                path_input = self.cache_path + "/{}_".format(k) + signature
                self.mem_maps[k] = np.memmap(path_input, dtype='int', mode='w+',
                                             shape=(self.number_instances, self.max_seq_len))
