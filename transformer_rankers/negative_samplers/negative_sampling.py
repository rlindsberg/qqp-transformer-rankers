...
if PYSERINI_USABLE:
    class BM25NegativeSamplerPyserini():
        ...
        def __init__(self, candidates, num_candidates_samples, path_index, sample_data, anserini_folder, set_rm3=False,
                     seed=42):
            random.seed(seed)
            self.candidates = candidates
            self.num_candidates_samples = num_candidates_samples
        ...
        def sample(self, query_str, relevant_docs, max_query_len=512):
            ...
            sampled_initial = [(hit.raw, hit.score) for hit in
                               self.searcher.search(query_str, k=self.num_candidates_samples)]
            was_relevant_sampled = False
            relevant_doc_rank = -1
            sampled = []
            scores = []
